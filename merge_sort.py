...
def merge_sort(lista):
    ...
    if len(lista) > 1:
        ...
        half_list_left = lista[:meio]
        # Tira uma cópia da segunda metade da lista
        half_list_right = lista[meio:]
        ...
        # partes menores
        half_list_left = merge_sort(half_list_left)
        half_list_right = merge_sort(half_list_right)
        ...
        while pos_esq < len(half_list_left) and pos_dir < len(half_list_right):

            comps += 1

            # O menor elemento está na sublista da esquerda
            if half_list_left[pos_esq] < half_list_right[pos_dir]:
                # "Desce" o elemento da esquerda para a lista ordenada
                ordenada.append(half_list_left[pos_esq])
                pos_esq += 1
            # O menor elemento está na sublista da direita
            else:
                # "Desce" o elemento da direita para a lista ordenada
                ordenada.append(half_list_right[pos_dir])
                pos_dir += 1
        ...
        # Sobra à esquerda
        if pos_esq < pos_dir: sobra = half_list_left[pos_esq:]
        # Sobra à direita
        else: sobra = half_list_right[pos_dir:]
